...
test_file="/home/ira/Dropbox/twitter/test_file_tagged2.txt"
...
####################   MINE   ####################################
        if predicted=="pos":  ##the twit according to the classifier contains a location
            ...
            #####################################################################
            cnt_list=counts.most_common(3)
            loc_test=cnt_list[0][0] 
            
            # check if one of the strings includes in herself the others two and that they include only letters
            if (cnt_list[0][0] in  cnt_list[2][0]) and  (cnt_list[1][0] in  cnt_list[2][0]):
                locl="".join(cnt_list[2][0].split())
                if locl.isalpha():
                    loc_test=cnt_list[2][0]    
            elif (cnt_list[0][0] in  cnt_list[1][0]) and  (cnt_list[2][0] in  cnt_list[1][0]):
                locl="".join(cnt_list[1][0].split())
                if locl.isalpha():
                    loc_test=cnt_list[1][0]  
